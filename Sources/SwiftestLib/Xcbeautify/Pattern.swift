enum Pattern: String {
  ...
  case codesignFramework = #"CodeSign\s((?:\ |[^ ])*.framework)\/Versions/A"#
    ...
    /// $3 = target
    case compile =
      #"Compile[\w]+\s.+?\s((?:\.|[^ ])+\/((?:\.|[^ ])+\.(?:m|mm|c|cc|cpp|cxx|swift)))\s.*\((in target: (.*)|in target '(.*)' from project '.*')\)"#
  #endif
  ...
  /// $3 = target
  case compileXib =
    #"CompileXIB\s(.*\/(.*\.xib))\s.*\((in target: (.*)|in target '(.*)' from project '.*')\)"#
  ...
  case compileStoryboard =
    #"CompileStoryboard\s(.*\/([^\/].*\.storyboard))\s.*\((in target: (.*)|in target '(.*)' from project '.*')\)"#
  ...
  /// $3 = target
  case copyHeader =
    #"CpHeader\s(.*\.h)\s(.*\.h) \((in target: (.*)|in target '(.*)' from project '.*')\)"#

  /// Regular expression captured groups:
  /// $1 = source file
  /// $2 = target file
  case copyPlist =
    #"CopyPlistFile\s(.*\.plist)\s(.*\.plist) \((in target: (.*)|in target '(.*)' from project '.*')\)"#

  /// Regular expression captured groups:
  /// $1 = file
  case copyStrings =
    #"CopyStringsFile\s(.*\.strings)\s(.*\.strings) \((in target: (.*)|in target '(.*)' from project '.*')\)"#

  /// Regular expression captured groups:
  /// $1 = resource
  case cpresource =
    #"CpResource\s(.*)\s\/(.*) \((in target: (.*)|in target '(.*)' from project '.*')\)"#
  ...
  case executed =
    #"\s*Executed\s(\d+)\stest[s]?,\swith\s(\d+)\sfailure[s]?\s\((\d+)\sunexpected\)\sin\s\d+\.\d{3}\s\((\d+\.\d{3})\)\sseconds"#
  ...
  /// $2 = target
  case generateDsym =
    #"GenerateDSYMFile \/.*\/(.*\.dSYM) \/.* \((in target: (.*)|in target '(.*)' from project '.*')\)"#
  ...
  /// $3 = time
    ...
    case testCaseMeasured =
      #"[^:]*:[^:]*:\sTest Case\s'(.*)\.(.*)'\smeasured\s\[Time,\sseconds\]\saverage:\s(\d*\.\d{3})(.*){4}"#
  #else
    case testCaseMeasured =
      #"[^:]*:[^:]*:\sTest Case\s'-\[(.*)\s(.*)\]'\smeasured\s\[Time,\sseconds\]\saverage:\s(\d*\.\d{3})(.*){4}"#
  #endif
  ...
  case parallelTestCasePassed =
    #"Test\s+case\s+'(.*)\.(.*)\(\)'\s+passed\s+on\s+'(.*)'\s+\((\d*\.(.*){3})\s+seconds\)"#
  ...
  case parallelTestCaseAppKitPassed =
    #"\s*Test case\s'-\[(.*)\s(.*)\]'\spassed\son\s'.*'\s\((\d*\.\d{3})\sseconds\)"#
  ...
  case parallelTestCaseFailed =
    #"Test\s+case\s+'(.*)\.(.*)\(\)'\s+failed\s+on\s+'(.*)'\s+\((\d*\.(.*){3})\s+seconds\)"#
  ...
  case phaseScriptExecution =
    #"PhaseScriptExecution\s(.*)\s\/.*\.sh\s\((in target: (.*)|in target '(.*)' from project '.*')\)"#

  /// Regular expression captured groups:
  /// $1 = file
  /// $2 = build target
  case processPch =
    #"ProcessPCH(?:\+\+)?\s.*\s\/.*\/(.*.pch) normal .* .* .* \((in target: (.*)|in target '(.*)' from project '.*')\)"#
